...
const handleSendStop = async (text?: string | MouseEvent) => {
  ...
  try {
    ...
    // 使用 Socket 进行流式聊天
    await chatStore.streamChatViaSocket()
    ...
    if (isVoiceInput.value) {
      ...
      if (lastMessage && lastMessage.role === 'assistant') {
        ...
        try {
          // 生成音频并保存 URL
          const audioUrl = await generateAudioUrl(lastMessage.content)

          if (audioUrl) {
            // 将音频 URL 添加到消息中，使用响应式更新
            const messageIndex = chatStore.messages.length - 1
            if (messageIndex >= 0) {
              // 使用store的方法更新消息属性，确保响应式更新
    ...
    // 注意：不要在这里重置 isVoiceInput，需要保持为 true 以显示语音条

    // 在持续模式下，AI回答完成后重新开始监听
    if (isContinuousMode.value && isVoiceInput.value) {
      console.log('🎤 AI回答完成，准备重新开始监听')
      setTimeout(async () => {
...
const processTextForAudio = (text: string): string => {
  ...
  if (cleanText.length <= maxLength) {
    console.log('🎤 文本在100字符以内（' + cleanText.length + '字符），直接使用:', cleanText)
    return cleanText
  }

  // 文本超过100字符，查找第一个句号的位置
  let firstSentenceEndIndex = -1
  for (const ending of sentenceEndings) {
    const index = cleanText.indexOf(ending)
    if (index !== -1 && (firstSentenceEndIndex === -1 || index < firstSentenceEndIndex)) {
      firstSentenceEndIndex = index
    }
  }

  // 如果找到句号，截取到第一个句号
  if (firstSentenceEndIndex !== -1) {
    const shortText = cleanText.substring(0, firstSentenceEndIndex + 1)
    const finalText = shortText + '具体内容请看文字版'
    console.log('🎤 文本超过100字符，截取到第一个句号（长度=' + shortText.length + '）:', finalText)
    return finalText
  }
  ...
  const finalText = shortText + '...具体内容请看文字版'
  console.log('🎤 文本超过100字符且无句号，截取前100字符:', finalText)
  return finalText
}
...
const generateAudioUrl = async (text: string): Promise<string | null> => {
  try {
    // 处理文本，如果太长则截取
    const processedText = processTextForAudio(text)
    console.log('🎤 开始生成音频:', processedText.substring(0, 50) + '...')
